...
OUTPUT_DIR   = "output"          # model, json, plots saved here
...
def prepare_features(train_df, test_df):
    ...
    TARGET    = "Arrest"
    DROP_COLS = ["Year"]   # used for splitting only
    ...
    # Align columns (guard against any mismatch)
...
def train_baseline(X_train, y_train, X_test, y_test, class_weight):
    print("\n" + "=" * 60)
    print("STEP 4 — Baseline Random Forest (default params)")
    print("=" * 60)
...
# ────────────────────────────────────────────────────────────
def tune_hyperparameters(X_train, y_train, class_weight):
    print("\n" + "=" * 60)
    print("STEP 5 — Hyperparameter tuning (RandomizedSearchCV)")
    print(f"         n_iter={N_ITER}, cv={CV_FOLDS}-fold, scoring=roc_auc")
    print("=" * 60)
...
def train_final_model(X_train, y_train, X_test, y_test,
    ...
    print(f"  Running {CV_FOLDS}-fold CV on final model...")
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    plt.suptitle("Random Forest — Confusion Matrix (Test Set)",
                 fontsize=13, y=1.02)
    plt.tight_layout()
...
def plot_roc_pr(y_test, y_proba):
    ...
    RocCurveDisplay.from_predictions(
        ...
        name="Random Forest (Tuned)", color="#2196F3")
    ...
    PrecisionRecallDisplay.from_predictions(
        ...
        name="Random Forest (Tuned)", color="#FF5722")
    ...
    plt.suptitle("Random Forest — Discrimination Curves (Test Set)",
                 fontsize=13)
    plt.tight_layout()
...
def plot_cv_scores(cv_scores):
    ...
    ymin = max(0, cv_scores.min() - 0.05)
    ymax = min(1, cv_scores.max() + 0.05)
    ax.set_ylim(ymin, ymax)
    ax.set_ylabel("AUC-ROC")
    ax.set_title(f"{CV_FOLDS}-Fold Cross-Validation AUC-ROC")
    ax.legend()
...
def plot_feature_importance(rf_final, feature_cols):
    importances = pd.Series(rf_final.feature_importances_,
                            index=feature_cols)
    top30 = importances.nlargest(30).sort_values()

    color_map = {
        "Primary Type"        : "#2196F3",
        "Location Description": "#FF9800",
        "District"            : "#4CAF50",
    }
    colors = []
...
def plot_calibration(y_test, y_proba):
    ...
    ax.plot(prob_pred, prob_true, "s-", color="#2196F3",
            label="Random Forest")
...
def plot_score_distribution(y_test, y_proba):
    ...
    ax.hist(y_proba[y_test == 0], bins=50, alpha=0.6,
            color="#4C72B0", label="No Arrest (0)", density=True)
    ax.hist(y_proba[y_test == 1], bins=50, alpha=0.6,
            color="#DD8452", label="Arrest (1)", density=True)
    ax.set_xlabel("Predicted Probability of Arrest")
...
def plot_baseline_vs_tuned(base_metrics, tuned_metrics):
    ...
    b1 = ax.bar(x - width/2, base_vals,  width, label="Baseline RF",
                color="#78909C", edgecolor="white")
    b2 = ax.bar(x + width/2, tuned_vals, width, label="Tuned RF",
                color="#1976D2", edgecolor="white")
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylim(0, 1.1)
    ax.set_ylabel("Score")
    ax.set_title("Baseline vs Tuned Random Forest — Test Set Metrics")
    ax.legend()
    ...
    plt.tight_layout()
    out = os.path.join(PLOT_DIR, "08_baseline_vs_tuned.png")
    plt.savefig(out, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved → {out}")


def run_all_plots(y_test, y_pred, y_proba, y_pred_base, y_proba_base,
                  cv_scores, rf_final, feature_cols,
                  base_metrics, tuned_metrics):
    print("\n" + "=" * 60)
    ...
    plot_baseline_vs_tuned(base_metrics, tuned_metrics)


# ────────────────────────────────────────────────────────────
# 8.  SAVE MODEL & METRICS
# ────────────────────────────────────────────────────────────
def save_artifacts(rf_final, feature_cols, tuned_metrics,
...
def predict_arrest(district, location_desc, primary_type,
                   weekday, month, hour,
                   year=2025, domestic=0,
                   model=None, feature_cols=None):
    """
    Predict arrest probability for a single crime event.

    Parameters
    ----------
    district      : int  — Police district (e.g. 8)
    location_desc : str  — Location description (e.g. 'STREET')
    primary_type  : str  — Crime type (e.g. 'THEFT')
    weekday       : int  — 0=Monday … 6=Sunday
    month         : int  — 1–12
    hour          : int  — 0–23
    year          : int  — prediction year (default 2025)
    domestic      : int  — 1=domestic, 0=non-domestic

    Returns
    -------
    dict : prediction (0/1), prediction_label, arrest_probability
    """
    ...
    for feat, val in scalar_feats.items():
    ...
    for col in [f"District_{district}",
        ...
        if col in row.columns:
            ...
            print(f"  Warning: '{col}' not in training features "
                  f"— treated as unseen category (zeros)")
...
def run_inference_examples(rf_final, feature_cols):
    ...
    examples = [
        dict(district=8,  location_desc="STREET",
             primary_type="THEFT",
             weekday=4, month=6,  hour=14, domestic=0),
        dict(district=11, location_desc="RESIDENCE",
             primary_type="BATTERY",
             weekday=5, month=12, hour=23, domestic=1),
        dict(district=1,
             location_desc="PARKING LOT / GARAGE (NON RESIDENTIAL)",
             primary_type="MOTOR VEHICLE THEFT",
             weekday=1, month=3,  hour=3,  domestic=0),
    ]

    for i, ex in enumerate(examples, 1):
        result = predict_arrest(**ex, model=rf_final,
...
def main():
    ...
    # 4. Baseline
    rf_base, y_pred_base, y_proba_base, base_metrics = \
        train_baseline(X_train, y_train, X_test, y_test, class_weight)

    # 5. Tune
    best_params, cv = tune_hyperparameters(X_train, y_train, class_weight)

    # 6. Final model
    rf_final, y_pred, y_proba, tuned_metrics, cv_scores = \
        train_final_model(X_train, y_train, X_test, y_test,
                          best_params, class_weight, cv)

    # 7. Plots
    run_all_plots(y_test, y_pred, y_proba,
                  y_pred_base, y_proba_base,
                  cv_scores, rf_final, feature_cols,
                  base_metrics, tuned_metrics)

    # 8. Save
    save_artifacts(rf_final, feature_cols, tuned_metrics,
                   cv_scores, best_params)

    # 9. Inference demo
    run_inference_examples(rf_final, feature_cols)
